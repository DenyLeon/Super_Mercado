...
public:
	...
	Cliente(string ic, string nom, string ape, string n, string tel, int gen, string ce, string fi) {
		idcliente = ic;
		nombres = nom;
		apellidos = ape;
		nit = n;
	...
	void crear() {
		...
		if (cn.getConectar()) {
			string insert = "INSERT INTO `proyecto`.`clientes`(nombres, apellidos, NIT, genero, telefono, correo_electronico, fecha_ingreso)VALUES('" + nombres + "','" + apellidos + "','" + nit + "'," + gen + ",'" + telefono + "','" + correo_electronico + "','" + fechaingreso + "'); ";
	...
	void Actualizar() {
		...
		string gen = to_string(genero);
		if (cn.getConectar()) {
			string update = "UPDATE `clientes` SET `nombres` = '" + nombres + "', `apellidos` = '" + apellidos + "', `NIT` = '" + nit + "', `genero` = " + gen + ",  `telefono` = '" + telefono + "', `correo_electronico` = '" + correo_electronico + "', `fecha_ingreso` = '" + fechaingreso + "' WHERE(`idCliente` = '" + idcliente + "');";
		...
		if (cn.getConectar()) {
			...
			if (!q_estado) {
				...
				while (fila = mysql_fetch_row(resultado))
				{
					...
					cout << "Nombre:" << fila[0] << fila[1] << endl; 
