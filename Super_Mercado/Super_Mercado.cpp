...
int main()
{
	...
	string idempleado, nombres, apellidos, DPI, fecha_nacimiento, fecha_inicio_labores, fechaingreso;
	string idcliente, correo_electronico;
	string idventa, no_factura, serie, fecha_factura, idventa_detalle, idproducto, cantidad;
	string idcompradetalle, costounitario;
	int idProducto = 0, genero=0, idMarca = 0, precio_costo = 0, precio_venta = 0, existencia = 0, precio_unitario,suma=0, menu = 0, smenu = 0, fprincipal =0;
	Producto p = Producto();
	Marcas m = Marcas();
	compra c = compra();
	...
	do {
		ofstream printer;
		ofstream arch;
		arch.open("Factura.txt", ios::out | ios::app);
		...
		case 8:
			...
			case 1:
				...
				for (int x = 0; x <= 62; x++)
				{
					...
					cout << '\n' << endl;
					cout << "   No. Factura:  " << no_factura << endl;
					cout << "   fecha:  " << fecha_factura << endl;
					cout << "   No. Serie " << serie << endl;
					cout << "   "; cl.buscar();
					cout << " \n";
					cout << "   "; p.buscar();
					cout << " \n";
				...
				arch << "                   	DATOS DE FACTURA                      \n";
				arch << "____________________________________________________________________\n";
				arch << "No.factura: " << no_factura << "                                             \n";
				arch << "____________________________________________________________________\n";
				arch << "Nit: " << serie << "                                                              \n";
				arch << "____________________________________________________________________\n";
				arch << "Nit: "<<nit<<"                                                              \n";
				...
				arch << "                             PRODUCTO                              \n";
				arch << "____________________________________________________________________\n";
				arch << idproducto << ""<<producto<<"" << precio_unitario << "\n";
				arch << "____________________________________________________________________\n";
				arch << "Total: " << suma << " \n";
				arch << "____________________________________________________________________\n";
				arch << std::put_time(&tm, "--------------------------%d/%m/%Y %H:%M:%S-----------------------") << std::endl;
				arch.close();
				printer.open("arch");
				printer.close();
