...
public:
	...
	Empleado(string emp, string nom, string ape, string dir, string tel, string d, int gen, string fn, string ip, string fil, string fi) {
		idempleado = emp;
		nombres = nom;
		apellidos = ape;
		direccion = dir;
	...
	void crear() {
		...
		if (cn.getConectar()) {
			string insert = "INSERT INTO `proyecto`.`empleados`(nombres, apellidos, direccion, telefono, DPI, genero, fecha_nacimiento, idPuesto, fecha_inicio_labores, fecha_ingreso)VALUES('" + nombres + "','" + apellidos + "','" + direccion + "','" + telefono + "','" + DPI + "'," + gen + ",'" + fecha_nacimiento + "','" + idpuesto + "','" + fecha_inicio_labores + "','" + fechaingreso + "'); ";
	...
	void Actualizar() {
		...
		string gen = to_string(genero);
		if (cn.getConectar()) {
			string update = "UPDATE `empleados` SET `nombres` = '" + nombres + "', `apellidos` = '" + apellidos + "', `direccion` = '" + direccion + "', `telefono` = '" + telefono + "', `DPI` = '" + DPI + "', `genero` = " + gen + ", `fecha_nacimiento` = '" + fecha_nacimiento + "', `idPuesto` = '" + idpuesto + "', `fecha_inicio_labores` = '" + fecha_inicio_labores + "', `fecha_ingreso` = '" + fechaingreso + "' WHERE(`idEmpleado` = '" + idempleado + "');";
